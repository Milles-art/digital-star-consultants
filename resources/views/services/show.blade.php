@extends('layouts.app', [
    'title' => $service->name,
    'metaDescription' => \Illuminate\Support\Str::limit(strip_tags($service->description ?? $service->name), 155),
])
...
@section('content')
    <section class="border-b border-line bg-gradient-to-b from-sky/40 to-paper">
        <div class="shell py-10 lg:py-14">
            <nav class="text-sm font-semibold text-muted" aria-label="Breadcrumb">
                <a href="{{ route('public.services.index') }}" class="hover:text-ink">Services</a>
            ...
            <div class="mt-6 grid gap-8 lg:grid-cols-[1fr_320px] lg:items-start">
                <div>
                    <p class="eyebrow">{{ $service->category->name ?? 'Service' }}</p>
                ...
                <div class="rounded-3xl border border-line bg-white p-5 shadow-sm">
                    <dl class="space-y-3 text-sm">
                        <div class="flex justify-between gap-4">
                            <dt class="font-bold text-muted">Price</dt>
                            <dd class="font-black text-ink">{{ $service->formatted_price }}</dd>
                        </div>
                        <div class="flex justify-between gap-4">
                            <dt class="font-bold text-muted">Duration</dt>
                            <dd class="font-black text-ink">{{ $service->duration }}</dd>
                        </div>
                        <div class="flex justify-between gap-4">
                            <dt class="font-bold text-muted">Fields</dt>
    ...
    <section class="shell py-12 lg:py-16">
        <div class="mx-auto max-w-2xl">
            <div class="rounded-3xl border border-line bg-white p-6 shadow-sm sm:p-8">
                <h2 class="text-xl font-black text-ink">Submit a request</h2>
                <p class="mt-2 text-sm text-muted">
                    Fill in your details below. After you submit, you will receive a reference number to track status — no login required.
                </p>
                ...
                >
                    ...
                    <fieldset class="space-y-4">
                        <legend class="text-xs font-bold uppercase tracking-[0.16em] text-blue">Your details</legend>
                        ...
                        <div>
                            ...
                            <input id="customer_name" name="customer_name" type="text" required maxlength="120"
                                class="field-input" placeholder="As on your ID">
                        </div>
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input id="customer_phone" name="customer_phone" type="tel" required maxlength="30"
                                    class="field-input" placeholder="+255 …">
                            </div>
                            <div>
                                <label class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-muted" for="customer_email">
                                    Email
                                </label>
                                <input id="customer_email" name="customer_email" type="email" maxlength="180"
                                    class="field-input" placeholder="Optional">
                            </div>
                        </div>

                        <div>
                            <label class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-muted" for="preferred_date">
                                Preferred date
                            </label>
                        ...
                        <div>
                            <label class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-muted" for="customer_notes">
                                Notes
                            </label>
                            <textarea id="customer_notes" name="customer_notes" rows="3" maxlength="1000"
                                class="field-input" placeholder="Anything else we should know?"></textarea>
                        </div>
                    </fieldset>
                    ...
                    @if ($service->fields->isNotEmpty())
                        <fieldset class="space-y-4 border-t border-line pt-6">
                            <legend class="text-xs font-bold uppercase tracking-[0.16em] text-blue">Service details</legend>
                                    ...
                                    @switch ($field->field_type)
                                                ...
                                                @foreach ($field->getOptionsArray() as $optKey => $optLabel)
                                                    <label class="flex items-center gap-2 text-sm font-semibold text-ink">
                                                        <input type="radio" name="{{ $name }}" value="{{ $optKey }}"
                                                            @checked($field->default_value == $optKey)
                                                            @required($required)
                                        ...
                                        @case('file')
                                            <input
                                                ...
                                                class="block w-full text-sm text-muted file:mr-4 file:rounded-full file:border-0 file:bg-sky file:px-4 file:py-2 file:text-sm file:font-bold file:text-blue hover:file:bg-blue/10"
                                            >
                                            <p class="mt-1 text-xs text-muted">PDF, JPG, PNG, DOC, DOCX · max 10MB</p>
                                            @break
                    ...
                    <button type="submit" class="button-primary w-full sm:w-auto" data-submit-btn>
                        Submit request
                    </button>
                </form>
            </div>
    ...
    <style>
        ...
        .field-input:focus {
            border-color: #1557a6;
            box-shadow: 0 0 0 4px rgba(21, 87, 166, 0.1);
        }
    </style>
        ...
        document.getElementById('submission-form')?.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    if (json.errors) {
                        Object.values(json.errors).flat().forEach((m) => messages.push(m));
                    }
                    errBox.innerHTML = messages.map((m) => `<p>${m}</p>`).join('') || '<p>Could not submit. Please check the form.</p>';
                ...
                const d = json.data || {};
                okBox.innerHTML = `
                    <p class="font-black text-emerald-900">${json.message || 'Submission created successfully'}</p>
                    <p class="mt-2 text-emerald-800">Reference: <strong>${d.reference_number || '—'}</strong></p>
                    <p class="mt-1 text-emerald-800">Keep this number to track your request.</p>
                    ${d.tracking_url ? `<p class="mt-3"><a class="font-bold text-blue underline" href="${d.tracking_url}">Open tracking page</a></p>` : ''}
                `;
